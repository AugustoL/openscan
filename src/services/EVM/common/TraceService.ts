...
export class TraceService {
  ...
  async traceTransaction(txHash: string, tracerConfig?: any): Promise<TransactionTrace | null> {
    try {
      const result = await this.rpcClient.call('debug_traceTransaction', [
        txHash,
        tracerConfig || { disableMemory: false, disableStack: false, disableStorage: false }
      ]);
      return result;
    } catch (error) {
   ...
   */
  async traceBlockByNumber(blockNumber: number | 'latest'): Promise<TransactionTrace[] | null> {
    try {
   ...
   */
  async traceBlockByHash(blockHash: string): Promise<TransactionTrace[] | null> {
    try {
